...
proc report data = fp.kopanski_means_freq nowd 
  style(column header)  = [just = center]; *discovered how to justify by forcing SAS to provide error notes (did not like 'j' or 'justification');
  columns studio score_stat RTscore ADscore Action--Thriller;
  define studio / order 'Lead Studio Name' style=[just = left cellwidth=1.6in];
...
proc report data = fp.kopanski_means_freq nowd 
  style(column header)  = [just = center];
  ...
  compute stu_rating;
    ...
    if stu_rating ge 0 and stu_rating < 60 then call define('Studio', 'Style', 'Style = [backgroundcolor=cx980043]');
      else if stu_rating ge 60 and stu_rating < 70 then call define('Studio', 'Style', 'Style = [backgroundcolor=cxdd1c77]');
      else if stu_rating ge 70 and stu_rating < 80 then call define('Studio', 'Style', 'Style = [backgroundcolor=cxdf65b0]');
      else if stu_rating ge 80 and stu_rating < 90 then call define('Studio', 'Style', 'Style = [backgroundcolor=cxd7b5d8]');
      else if stu_rating ge 90 then call define('Studio', 'Style', 'Style = [backgroundcolor=cxf1eef6]');
  endcomp;
run;
