/*
 ...
 v4: Tested on VTL.
 v5: Fixed idents throughout code. Correction to v3 comment. OUT= statemenet was 
     removed from proc report and ods output was added to proc means. Earlier comment
     implied OUT = was originally in proc means. Tested on VTL.
*/
...
data HW8.blood_reshape;
  ...
  set HW8.blood_R2out;
    ...
    do i = 1 to dim(_Blood);
      do j = 1 to dim(_Blood,2);
        B_Stat[j] = _Blood[i,j];
        if i = 1 then TCode = 'WBC';
          else if i = 2 then TCode = 'CHOL';
      end;
      output;
    end;
