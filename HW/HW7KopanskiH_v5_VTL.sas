/*
 ...
 Class  = ST555               
 Project= HW07                
 Purpose = To create SAS datesets from raw EPA site pollution data.
 Date Created = 07APR2020     
 Date Modified = 08APR2020 v5
 v2: Modified data step in Section 4 to create two datasets (final and 100).
 v3: Modified data step in Section 4 to add 4 variables (day StCode 
     CountyCode SiteNum) back into the 100 data set.
 v4: Final VTL tested version. Reconfigured libraries for VTL, cleaned typos, 
     and fixed indents.
 v5: Fixed library and filename references to satisfy homework requirements. Parameterized some values.
     Tested on VTL server. 
*/
...
x 'cd S:\Documents\hk_user\HW7_Results';
libname HW7 ".";
filename HW7 ".";
...
data HW7.EPA01(drop = _: finaldate);
  ...
  finaldate = '31DEC2018'd;
  date = scan(_Day_, 1,,'lu') + finaldate;
...
data HW7.EPA03(drop = i initialdate finaldate diffdate);
  ...
  attrib 
    ...
    count     length = 8.;
  date = '01JAN2019'd;
  initialdate = '31DEC2018'd;
  finaldate = '01SEP2019'd;
  diffdate = finaldate - initialdate;
  aqsabb = 'CO';
...
create the final dataset to be used for future analysis. A second dataset of 
only complete daily records was also created. */
...
data HW7.HW7FinalKopanski(drop = day StCode CountyCode SiteNum initialdate) 
  HW7.HW7FinalKopanski100(drop = initialdate);
  attrib 
  ...
  if aqscode ne 44201 then do;
    day = put(date - initialdate, comma8.);
  end;
  if (inCollSites eq 1 and inMet eq 1) then output HW7.HW7FinalKopanski;
  if (inCollSites eq 1 and inMet eq 1 and percent eq 100) then output HW7.HW7FinalKopanski100;
run;
/* End of Section 4 */

/* Section 5: 
Create datasets from information derived from final dataset. 
Descripter dataset contains variable attributes. */
...
proc compare base = Results.HW7finalduggins
...
proc compare base = Results.HW7finalduggins100
